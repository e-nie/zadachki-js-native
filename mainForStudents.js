...
let deepCopySuperUser1 = {...superUser, friends:superUser.friends.map(item=> ({...item}))}
...
// let trueStudents = studen
// console.log(trueStudents)

//8. Nick женился. Выполните соответствующие преобразование массива
// students (map)
let studentsWithMarriedNick = students.map(item=> {
    item.name === 'Nick' ? {...item, isMarried:true }: item
})
...
let superUserCorrect1 = {...superUser, friends:superUser.friends.filter(item=> item.id !== 1)}

//10. поменяйте объекту с id=2 из массива  friends значение св-ва name на
// "Donald"
let superUserCorrect2 = {...superUser, friends: superUser.friends.map(item=> item.id === 2 ? {...item, name: 'Donald'} : item)}
...
for (let i = 1; i < students.length - 1; i++) {
    if(students[i].scores > bestStudents.scores) {
