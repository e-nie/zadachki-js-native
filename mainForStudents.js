...
deepCopyStudents.teacher = 'madam';
...
let deepCopySuperUser = {
    ...superUser,
    friends: [...superUser.friends]
};

let deepCopySuperUser0 = structuredClone(superUser);
deepCopySuperUser.brains = true;
// console.log(deepCopySuperUser);
deepCopySuperUser0.brains = false;
// console.log(deepCopySuperUser0);
// console.log(superUser);

let deepCopySuperUser1 = { ...superUser, friends: superUser.friends.map(item => ({ ...item })) };  //❗️

// console.log(deepCopySuperUser1);
//4. Отсортируйте students по успеваемости (лучший идёт первым)(sort)
let sortedByScores = [...students].sort((a, b) => b.scores > a.scores ? -1 : 1);
...
let trueStudents = [...students, { ...students.map(item => ({ ...item, isStudent: true })) }];
...
let superUserCorrect1 = { ...superUser, friends: [...superUser.friends.filter(el => el.id !== 1 ? el : '')] };
...
let superUserCorrect2 = {
    ...superUser,
    friends: superUser.friends.map(item => item.id === 2 ? { ...item, name: 'Donald' } : item)
};
...
let scoresSum = students.reduce((acc, curr) => {
...
//             if(i !== j) {
//                 otherStudents.push(students[j].name)
//             }
//         }
//         currentStudent.friends = otherStudents
//     }
//     return students
// };
...
const addFriends = () => {
    students.forEach(student=> {
        // Создаем копию массива students без текущего студента
        const friends = students.filter(s=> s.name !== student.name)
        student.friends = friends.map(friend => friend.name)
    })
    return students
}
...
// {...superUser, friends:superUser.friends.filter(item=> item.id !== 1)}
